...
class MultiheadAttention_causal(Module):
    r"""Allows the model to jointly attend to information
    from different representation subspaces as described in the paper:
    `Attention Is All You Need <https://arxiv.org/abs/1706.03762>`_.
    ...
    def __init__(self, embed_dim, num_heads, objective_matrix,dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False,
                 kdim=None, vdim=None, batch_first=False, device=None, dtype=None) -> None:
        factory_kwargs = {'device': device, 'dtype': dtype}
        super().__init__()
        self.embed_dim = embed_dim
    ...
    Outputs:
        ...
        is_batched = query.dim() == 3
        # value*wc
        tensor = torch.from_numpy(self.objective_matrix)
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        # 将张量移动到目标设备
        objective_tensor = tensor.to(device)
        value = torch.matmul(objective_tensor.float(),value)
